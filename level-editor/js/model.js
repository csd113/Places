...
const CORE_MATERIALS = {
  ...
  // Decal sheets are generated by the renderer and drawn in the decal pass.
  // They are surface markings, not wall/floor/ceiling materials.
  'core:decal_test_01': {
    id: 'core:decal_test_01',
    name: 'Decal Test',
    category: 'decal',
    color: '#f2f2ee',
    border: '#7c7c74'
...
/** Decal surface ids, shared by the model, the inspector and validation. */
const DECAL_SURFACES = ['floor', 'ceiling', 'wall_north', 'wall_south', 'wall_east', 'wall_west'];
/** Largest decal edge the game accepts, in metres (mirrors MAX_DECAL_SIZE_M). */
const DECAL_MAX_SIZE = 10.0;
...
class CeilingLight {
  constructor(data = {}) {
    ...
    // Emitted light colour as [r, g, b] fractions, or null for the game's
    // documented warm default. The panel and the baked illumination both use it.
...
class Decal {
  constructor(data = {}) {
    this.id = data.id || generateUniqueId('decal');
    this.x = Number(data.x ?? 0);
    this.y = Number(data.y ?? 0);
    this.z = Number(data.z ?? 0);
    this.width = Number(data.width ?? 1);
    ...
    this.rotation_degrees = Number(data.rotation_degrees ?? 0);
    this.material = data.material || 'core:decal_test_01';
    this.surface = data.surface || 'wall_south';
  }
  ...
  duplicate() {
    const copy = this.clone();
    copy.id = generateUniqueId('decal');
    return copy;
  }
...
class Level {
  constructor(data = {}) {
    ...
    this.decals = (data.decals || []).map(d => new Decal(d));
  ...
  clone() {
    return new Level({
      ...
      decals: this.decals.map(d => d.clone()),
  ...
  toJSON() {
    ...
    if (this.decals.length > 0) {
      result.decals = this.decals.map(d => d.toJSON());
    }
...
function validateLevel(level) {
  ...
  level.ceiling_lights.forEach((l, i) => {
    ...
    if (l.color !== null && l.color !== undefined) {
      if (!Array.isArray(l.color) || l.color.length !== 3) {
        errors.push(`Ceiling light ${i} colour must be an [r, g, b] array`);
  ...
  decals.forEach((d, i) => {
    if (![d.x, d.y, d.z, d.rotation_degrees].every(Number.isFinite)) {
      errors.push(`Decal ${i} position and rotation must be finite numbers`);
    }
    if (!Number.isFinite(d.width) || !Number.isFinite(d.height)) {
      errors.push(`Decal ${i} size must be finite numbers`);
      return;
    }
    if (d.width <= 0 || d.height <= 0) {
      errors.push(`Decal ${i} width and height must be positive numbers`);
    } else if (d.width > DECAL_MAX_SIZE || d.height > DECAL_MAX_SIZE) {
      errors.push(`Decal ${i} is larger than the ${DECAL_MAX_SIZE} m limit (${d.width} x ${d.height})`);
    }
    if (!d.material || !String(d.material).trim()) {
      errors.push(`Decal ${i} must reference a non-empty material id`);
    } else {
      checkMaterial(d.material, `Decal ${i}`);
    }
    if (!DECAL_SURFACES.includes(d.surface)) {
      errors.push(`Decal ${i} surface "${d.surface}" is not one of ${DECAL_SURFACES.join(', ')}`);
    }
  });
