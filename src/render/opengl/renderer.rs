...
use super::shaders::{
    DECAL_ALPHA_CUTOFF, DECAL_FRAGMENT_SHADER_SRC, DECAL_POLYGON_OFFSET,
    EMISSION_MASK_TEXTURE_UNIT, LIGHTMAP_PAGE_SLOTS, LIGHTMAP_TEXTURE_UNIT,
    LIGHTMAP_TEXTURE_UNIT_1, NORMAL_MAP_TEXTURE_UNIT, PRESENT_FRAGMENT_SHADER_SRC,
    PRESENT_TEXTURE_UNIT, PRESENT_VERTEX_SHADER_SRC, REFLECTION_PLANAR_TEXTURE_UNIT,
    REFLECTION_PROBE_TEXTURE_UNIT, SCENE_ATTRIB_COLOR, SCENE_ATTRIB_COUNT, SCENE_ATTRIB_HANDEDNESS,
    SCENE_ATTRIB_LIGHTMAP_PAGE, SCENE_ATTRIB_LIGHTMAP_UV, SCENE_ATTRIB_NORMAL, SCENE_ATTRIB_POS,
    SCENE_ATTRIB_TANGENT, SCENE_ATTRIB_UV, SCENE_TEXTURE_UNIT, VERTEX_SHADER_SRC,
...
use crate::materials::{MaterialAlpha, MaterialEmission, MaterialReflection, ReflectionMode};
...
use crate::render::common::camera::RenderCamera;
use crate::render::common::decals::{DECAL_ATLAS_SIZE, generate_decal_atlas};
use crate::render::common::dynamic::{DynamicMesh, DynamicScene, DynamicUpdate};
use crate::render::common::frame::{FrameState, PreparedFrame};
use crate::render::common::materials::{
    BatchPass, EmissionRouting, MaterialRenderState, ScenePass, TranslucentDraw, TranslucentSource,
    batch_pass_for, collect_translucent_draws, emission_routing,
};
...
use crate::render::common::{
    BatchRange, DECAL_EXTERNAL_BASE, LevelMesh, MaterialIndex, MeshChunk, MeshPacker, PackedVertex,
    PropMeshBatch, SCENE_FAR_M, SCENE_NEAR_M, StaticBatch, SurfaceKey, SurfaceKind, Vertex,
    VertexLayout, decal_external_sheet_ids, exact_layout, packed_layout, spatial_cell_grid,
};
use crate::spatial::Frustum;

use super::framebuffer::{PRESENT_QUAD, SceneTarget, present_matrix};

/// Whether the offscreen scene path should start enabled.
///
/// On by default. `PLACES_NO_OFFSCREEN=1` forces the historical
...
fn offscreen_requested_from_env() -> bool {
    std::env::var("PLACES_NO_OFFSCREEN").as_deref() != Ok("1")
}
...
pub struct SurfaceState {
    ...
    /// the material.
    pub reflection_plane: Option<u16>,
    /// How this surface's emission moves over time, if the level animates it.
    pub emission_animation: Option<EmissionAnimation>,
}
...
impl SurfaceState {
    ...
    /// the same terms.
    pub const fn plain(texture: glow::Texture) -> Self {
        Self {
            texture,
            normal: None,
...
/// The program slot one scene pass owns in the uniform-location tables.
#[must_use]
const fn scene_pass_index(pass: ScenePass) -> usize {
    match pass {
        ScenePass::World => 0,
        ScenePass::Cutout => 1,
    }
...
pub struct SceneAttributePointer {
...
pub const WHITE_SHEET_PNG: &[u8] = include_bytes!("../../../assets/core/textures/white_01.png");
...
pub fn load_white_sheet() -> Result<crate::loader::RawImage, String> {
...
unsafe fn create_post_process(gl: &glow::Context) -> Option<super::postprocess::PostProcess> {
    match unsafe {
        super::postprocess::PostProcess::create(
            gl,
            crate::render::common::postprocess::PostSettings::for_profile(
                crate::quality::QualityProfile::DEFAULT,
            )
            .with_bloom(true),
        )
    } {
    ...
    unsafe {
        ...
        for pass in ScenePass::ALL {
            if let (Some(uniforms), Some(program)) = (
                uniforms.get(scene_pass_index(pass)),
                programs.get(scene_pass_index(pass)),
            ) {
...
unsafe fn scene_attribute_locations(
    ...
    let Some(&program) = programs.get(scene_pass_index(ScenePass::World)) else {
...
pub struct Renderer {
    ...
    /// `PLACES_NO_OFFSCREEN=1` benchmark override).
    ...
    /// that image (see [`crate::render::common::reflections::is_mirror_range`]).
    ...
    /// resident.
    black_cube: glow::Texture,
    /// The current level's reflection routing and switches: which materials
    /// read which source. Renderer-neutral; the GPU targets live in
    /// [`ReflectionTargets`].
    reflections: crate::render::common::reflections::Reflections,
    /// Baked probe cubemaps and the planar target: the GPU half of the
    /// reflection system.
    reflection_targets: super::reflections::ReflectionTargets,
}
...
impl Renderer {
    ...
    pub fn new(window: &sdl2::video::Window, video: &sdl2::VideoSubsystem) -> Result<Self, String> {
        super::context::request_window_attributes(video);

        let gl_context = if let Ok(ctx) = window.gl_create_context() {
            ctx
        } else {
            super::context::request_fallback_window_attributes(video);
    ...
    fn from_startup(
        ...
        let mut renderer = Self {
            ...
            // (including the `PLACES_NO_BLOOM` / `PLACES_NO_REFLECTIONS`
            ...
            reflections: crate::render::common::reflections::Reflections::default(),
            reflection_targets: super::reflections::ReflectionTargets::default(),
        };
    ...
    fn refresh_post_settings(&mut self) {
        let settings = crate::render::common::postprocess::PostSettings::for_profile(self.quality)
    ...
    ///
    /// Used by `PLACES_CAPTURE` for reproducible desktop render checks.
    /// Call it after drawing and before swapping buffers.
    /// # Errors
    ///
    ...
    fn resolve_scene_extras(
        ...
        let reflections = materials
            ...
            .collect::<Vec<_>>();
        let routing = crate::render::common::reflections::routing_from_mesh(
            mesh,
            &reflections,
            reflections.len(),
        );
        ...
        // The geometry the old probes were baked for no longer exists: drop
        // them before installing the new routing.
        unsafe { self.reflection_targets.clear_probes(&self.gl) };
    ...
    /// [`scene_target_size`]), so nothing is stretched, and a target
    ...
    pub fn render_scene(&mut self, camera: RenderCamera) {
        // Everything that does not depend on a GPU resource is decided here,
        // without GL: the scene render size, the view-projection and frustum,
        // the mirror plane and the probe this frame reads.
        let Some(mut frame) = PreparedFrame::plan(&FrameState {
            camera,
            drawable: self.drawable_size,
            quality: self.quality,
            offscreen_enabled: self.offscreen_enabled,
            offscreen_failed: self.offscreen_failed,
            culling: self.culling_enabled,
            reflections: &self.reflections,
            probe_positions: self.reflection_targets.probe_positions(),
        }) else {
            return;
        };
        let offscreen = self.ensure_scene_target(frame.target_size);
        frame.begin_scene(offscreen, &self.reflections.routing);
        let render_size = frame.render_size;
        self.camera_pos = frame.camera.position;
        self.scene_mvp = frame.view_projection;
        ...
        if offscreen {
            self.capture_reflections(&frame);
            ...
            unsafe { self.bind_reflection_textures(frame.probe, None) };
        }
        ...
        // last.
        let totals = self.draw_scene_body(&frame.view_projection, &frame.frustum, frame.culling);

        let bloom = if offscreen {
            self.capture_bloom(&frame)
        } else {
            None
        };
    ...
    fn ensure_scene_target(&mut self, size: Option<DrawableSize>) -> bool {
        ...
        if self.scene_target.is_none() || self.scene_target_size != size {
            ...
            match unsafe { SceneTarget::create(&self.gl, size) } {
                Ok(target) => {
    ...
    unsafe fn bind_reflection_textures(
        &mut self,
        probe: Option<usize>,
        planar: Option<glow::Texture>,
    ) {
        let probe = probe
            .and_then(|index| self.reflection_targets.cube(index))
            .unwrap_or(self.black_cube);
        unsafe {
            self.gl
                .active_texture(super::shaders::texture_unit(REFLECTION_PROBE_TEXTURE_UNIT));
            self.gl.bind_texture(glow::TEXTURE_CUBE_MAP, Some(probe));
            self.gl
                .active_texture(super::shaders::texture_unit(REFLECTION_PLANAR_TEXTURE_UNIT));
            self.gl
    ...
    /// shows.
    fn capture_reflections(&mut self, frame: &PreparedFrame) {
        self.active_plane = None;
        let render_size = frame.render_size;
        let probe = frame.probe;
        let plane_index = frame.planar_plane;
        let mut planar_texture = None;
        if let Some(index) = plane_index
            && self.reflections.planar_wanted()
        {
            let Some(plane) = self.reflections.routing.planes.get(index).copied() else {
                unsafe { self.bind_reflection_textures(probe, None) };
                return;
            };
            let ready = unsafe { self.reflection_targets.ensure_planar(&self.gl, render_size) };
            if !ready {
                // A context that cannot give us a reflection target loses the
                // planar pass for the session.
                self.reflections.disable_planar();
            }
            if ready {
                let mirror =
                    crate::render::common::reflections::mirror_matrix(plane.normal, plane.offset);
                let [nx, ny, nz] = plane.normal;
                // `glam` matrix products are per-element `f32` arithmetic with
                // no overflow or panic path.
                #[allow(clippy::arithmetic_side_effects)]
                let mirrored_mvp = frame.view_projection * mirror;
                let mirrored_camera = crate::render::common::reflections::mirror_point(
                    plane.normal,
                    plane.offset,
                ...
                if self.reflection_targets.planar().is_some() {
                    if let Some(planar) = self.reflection_targets.planar() {
                    ...
                    let _ = self.draw_scene_body(&mirrored_mvp, &frustum_mirrored, frame.culling);
        ...
        unsafe { self.bind_reflection_textures(probe, planar_texture) };
    }
    ...
    fn capture_bloom(&mut self, frame: &PreparedFrame) -> Option<glow::Texture> {
        // The bloom setting, the `PLACES_NO_BLOOM` startup override and a view
        ...
        let scene_depth = self.scene_target.as_ref().map(SceneTarget::depth_buffer);
        {
        ...
        let _ = self.draw_emissive_body(&frame.view_projection, &frame.frustum, frame.culling);
        let post = self.post.as_ref()?;
    ...
    fn resolve_scene(&mut self, bloom: Option<glow::Texture>) {
        let drawable = self.drawable_size;
        let Some(scene) = self.scene_target.as_ref().map(SceneTarget::color) else {
            return;
        };
        let post = self.post.as_ref();
    ...
    fn bake_reflection_probes(&mut self) {
        ...
        // a zero texture.
        unsafe { self.bind_reflection_textures(None, None) };
        ...
        for point in points.into_iter().take(wanted) {
            ...
            self.reflection_targets.push_probe(probe);
        }
        self.scene_mvp = glam::Mat4::IDENTITY;
        let millis = started.elapsed().as_secs_f64() * 1000.0;
        crate::logging::info(format!(
            "[reflections] baked {} probe(s) at {} texels/face in {millis:.1} ms",
            self.reflection_targets.probe_count(),
            self.reflection_targets.first_face_texels()
        ));
    }

    fn report_reflections(routing: &crate::render::common::reflections::ReflectionRouting) {
        crate::logging::info(format!(
    ...
    fn is_capture_mirror(&self, key: SurfaceKey) -> bool {
        self.reflection_capture
            && key.has_material()
            && crate::render::common::reflections::is_mirror_range(
    ...
    fn reflection_uniforms(&self, state: &SurfaceState) -> (f32, [f32; 3]) {
        ...
        match state.reflection.mode {
            ...
            ReflectionMode::Probe => {
                if self.reflection_targets.is_empty() {
                    (0.0, zero)
                } else {
                    (1.0, strength)
                }
    ...
    /// `PLACES_NO_LIGHTMAPS=1`), applied by `set_level`/`rebuild_level_geometry`.
    ...
    /// white sheet, so an unused unit always has a defined sample. This is the
    /// one signature that could carry a GL handle; it is deliberately private —
    /// atlas pages are uploaded and cleared by the backend, never by callers.
    fn set_lightmap_page(&mut self, slot: usize, texture: Option<glow::Texture>) {
    ...
    fn bind_lightmap_units(&mut self) {
        unsafe {
            self.gl
                .active_texture(super::shaders::texture_unit(LIGHTMAP_TEXTURE_UNIT));
            self.gl
                .bind_texture(glow::TEXTURE_2D, Some(self.lightmap_pages[0]));
            self.gl
                .active_texture(super::shaders::texture_unit(LIGHTMAP_TEXTURE_UNIT_1));
            self.gl
                .bind_texture(glow::TEXTURE_2D, Some(self.lightmap_pages[1]));
            self.gl
                .active_texture(super::shaders::texture_unit(SCENE_TEXTURE_UNIT));
        }
        self.render_stats.texture_binds = self.render_stats.texture_binds.saturating_add(2);
    }
    ...
    fn draw_dynamic_objects(
        ...
        for object in dynamic.objects() {
            ...
            let mvp = {
                ...
                // the operator impl (the same note `RenderCamera::view_projection`
                // carries).
                #[allow(clippy::arithmetic_side_effects)]
...
/// does not have). With `indexed` false (`PLACES_BENCH_NOINDEX`) every range is
...
/// and vertex layout.
pub fn pack_static_batches(mesh: &LevelMesh, indexed: bool) -> (MeshPacker, Vec<StaticBatch>) {
    let mut packer = MeshPacker::default();
