...
Rigged, animated **entity** assets live in the sibling
[`../entities/`](../entities/README.md) toolkit: the rigged-GLB writer, the
build scripts for the shipped entities and the parallel offline validators.
...
## Extended writer (multi-material, nodes, clips)

`glb.write_glb` accepts optional `submeshes`/`materials` tags recorded through
`PropBuilder.material(...)`, `nodes`/`clips` recorded through
`PropBuilder.nodes`/`PropBuilder.clips`, and writes one glTF primitive per
contiguous material run, TRS node hierarchies and LINEAR/STEP animation clips.
...
* `parts/signage.py` — `core:stop_sign` (painted octagon and a shared block
  font) and `core:exit_sign` (a green emissive face plus a housing).
* `parts/home.py` — `home:ball_light` (an emissive orb with its own material
  slot) and `home:wall_switch` (a three-node rigid animation, no skin).
* `tools/props/glyphs.py` — the shared 5x7 block font used by both the prop
  and the decal painters.

`build.py` skips a hand-authored skinned/animated GLB unless `--force` is
given; a toolkit-authored animated model marks itself
(`asset.extras.places_props_toolkit = 1`) so its own builder may rebuild it.

## Layout

| file                  | purpose                                                            |
| --------------------- | ------------------------------------------------------------------ |
| `build.py`            | builds every prop in the catalogue, validates, prints a budget report |
| `preview.py`          | software rasteriser: per-prop previews, contact sheets              |
...
Previews are written to `target/prop-previews/` (gitignored).
...
## Geometry-only maintenance

Some shipped static GLBs contain hand-finished geometry that the older part
builders do not reproduce. For geometry repairs, operate on the shipped GLB;
do not run a blanket rebuild over those assets. The reviewed repair pass is:

```sh
python3 tools/props/repair_geometry.py --report target/geometry-audit.json
python3 tools/props/repair_geometry.py --apply --report target/geometry-repairs.json
python3 -m unittest discover -s tools/props -p test_geometry.py
python3 tools/props/build.py --check
```

The first command is read-only. The pass covers the 32 static props listed in
`repair_geometry.NAMES`, preserving embedded PNG bytes, existing UVs, vertex
colors and material definitions. It corrects winding, closes specifically
reviewed component holes, joins the washer drum wall to its rim, and recesses
the flat-panel TV screen. Exact vertex records are deduplicated without welding
UV/color seams. Character rigs and the preceding 16 polished assets are excluded.
It is deliberately not a general-purpose hole filler: curtain backs, recessed
washer surfaces, surface planes and assembly joints need different treatment.
See [the repair report](../../docs/reports/geometry-repair.md) for the shipped
results, remaining assembly topology, and inspection renders.
...
opt-in closed, outward-facing primitives. Other props still use their existing
builders.
