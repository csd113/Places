# External texture / material validation (Goal 4.5)
...
* `render::tests::test_carpet_png_carries_the_metre_checker`
...
* `render::tests::the_residential_levels_resolve_their_stain_overlays`
...
for spec in "a_room_a:1.5,4.0,90" "b_overlay:2.0,7.0,0" "c_alpha_wall:5.0,4.0,90" \
  ...
  LIMINAL_LEVEL=texture_diagnostic LIMINAL_SPAWN="$spawn" \
    LIMINAL_CAPTURE="/tmp/places45/$name.png" ./target/release/liminal-rust
done
...
for spec in "rgb_warm:2.0,4.5,0:0,-38" "rgb_blue:6.0,4.5,180:180,-38" \
  ...
  LIMINAL_LEVEL=texture_diagnostic LIMINAL_BENCH=1 LIMINAL_SPAWN="$spawn" \
    LIMINAL_CAMERA="$cam" LIMINAL_CAPTURE="/tmp/places45/$name.png" ./target/release/liminal-rust
done
```
...
* the overlay run in `b_overlay` shows the orange/cyan NPOT material over the
  base wall with no coplanar fighting (Goal 2);
* the recess in `e_recess` sits below the floor with real transition faces, the
  ...
  elevated room (Goal 4);
* `rgb_warm` / `rgb_blue` show warm or cool casts on the same textures while
  `rgb_white` stays neutral (Goal 1);
* no magenta/black checker anywhere: any diagnostic pattern is a real
  resolution failure and the console names the material.

Legacy checks: `LIMINAL_LEVEL=vertical_diagnostic` shots from
`vertical-geometry-validation.md` and `LIMINAL_LEVEL=level_1` must look
structurally identical to the Goal 4 captures (Level 1 is artistic content and
...
are in the Goal 4.5 report. The Goal 4 walk itself
(`LIMINAL_LEVEL=vertical_diagnostic`, no spawn) still climbs 1.60 -> 3.60,
passes the shallow recess at 3.25 and stops at x ≈ 24.

## Creator replacement test (the phase's core requirement)

Prove that changing a PNG changes the game **without recompiling**:
