...
# The overall quality level and the three Advanced settings are independent, so
# each profile pins all four explicitly to its preset defaults (Low ->
# low/off/off, High -> high/full/full); a bare `PLACES_QUALITY` override would
# leave the advanced settings at the saved values.
mkdir -p "$STATE"
write_state() {
    profile=$1
    case "$profile" in
