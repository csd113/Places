...
(walls, floors, ceilings), the **decal sheets** and the diagnostic set, all
registered as file-backed `asset_type: "texture"` / `asset_type: "decal"`
entries in [`../../assets/catalog.json`](../../assets/catalog.json).
...
`--check` reads `assets/catalog.json` and, for every texture asset and every
file-backed decal sheet:
...
`build.py` merges them and refuses a duplicate id.
...
1. Add the painter and its `ART` entry in the theme's art module
   (`office_art.py`, `pool_art.py`, `decal_art.py`, or a new module registered
   in `build.py`).
...
3. Point a `material` entry's `texture` field at a texture
   (`source: "definition"`, optional `tile_metres`, `tint`), or place the decal
   id in a level's `decals`.
...
background is alpha 0 and the artwork is the silhouette plus its plate.
